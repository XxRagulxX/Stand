...
namespace Stand
{
	...
	{
	public:
		static inline const Particle particles[] = {
		...
		static constexpr int count = sizeof(particles) / sizeof(particles[0]);

		static void tryPlay(int idx, const rage::fvector3& pos, float scale = 1.0f)
		{
			...
			if (STREAMING::HAS_NAMED_PTFX_ASSET_LOADED(p.assetCategory))
			{
				...
				GRAPHICS::START_NETWORKED_PARTICLE_FX_NON_LOOPED_AT_COORD(
					p.rawName, pos.x, pos.y, pos.z, 0.0f, 180.0f, 0.0f, scale, true, true, true, false);
			}
		}

		static void play(int idx, const rage::fvector3& pos, float scale = 1.0f)
		{
			const Particle& p = particles[idx];
			STREAMING::REQUEST_NAMED_PTFX_ASSET(p.assetCategory);
			while (!STREAMING::HAS_NAMED_PTFX_ASSET_LOADED(p.assetCategory))
