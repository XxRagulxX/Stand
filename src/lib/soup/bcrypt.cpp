...
#define BLFRND(s,p,i,j,n) (i ^= F(s,j) ^ (p)[n])
	...
	void Blowfish_decipher(blf_ctx* c, uint32_t* xl, uint32_t* xr)
	{
		uint32_t Xl;
		uint32_t Xr;
		uint32_t* s = c->S[0];
...
#define CHAR64(c) branchless::trinary<uint8_t>((c) > 127, 255, index_64[(c)])
	...
	void encode_salt(char* salt, uint8_t* csalt, char minor, uint16_t clen, uint8_t logr)
	{
		salt[0] = '$';
	...
	*/
	void bcrypt_gensalt(char minor, uint8_t log_rounds, uint8_t* seed, char* gsalt)
	{
		if (log_rounds < 4)
			log_rounds = 4;
	...
	std::string bcrypt::hash(const std::string& password, unsigned rounds)
	{
		char salt[_SALT_LEN];
