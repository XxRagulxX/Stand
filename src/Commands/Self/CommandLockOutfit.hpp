...
namespace Stand
{
	...
	public:
		...
		explicit CommandLockOutfit(CommandList* parent);
	...
	private:
		static std::array<std::int16_t, COMPACT_OUTFIT_SIZE> buildCompactOutfit();
		static void applyCompactOutfit(const std::array<std::int16_t, COMPACT_OUTFIT_SIZE>& outfit);
	};
}
