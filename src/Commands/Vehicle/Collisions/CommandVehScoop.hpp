...
namespace Stand
{
    namespace
    {
        class CommandVehScoop : public CommandSlider
        {
            bool m_ticking = false;
        public:
            explicit CommandVehScoop(CommandList* parent)
                : CommandSlider(parent, LIT("Ramming Scoop"), CMDNAMES("vehscoop"), NOLABEL, 0, 2, 0) {}

            ~CommandVehScoop() override
            {
            ...
            void onTick() override
            {
                ...
                if (!model) return;
                if (value == 1)
                    CollFieldAt<uint8_t>(model, 0x57C + 15) |= static_cast<uint8_t>(1u << 7);
                else
                    CollFieldAt<uint8_t>(model, 0x57C + 15) &= static_cast<uint8_t>(~(1u << 7));
            }
